iceipiceApp.controller('cookShiftsController', function ($scope, $http, $state, $stateParams, authorizationService) {
    $scope.current.page = 1;
    
    $scope.shiftsInCalendar = [];
    $scope.user = authorizationService.getUser();

    $http.get('/api/cookShifts/forOne/' + $scope.user.id).success(function (data) {
        for (var i in data) {
            var d = data[i];
